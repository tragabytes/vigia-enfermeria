...
~~**RTVE** (`https://convocatorias.rtve.es/puestos-ofertados`): HTTP 200 pero **body_text=0** — SPA absoluta donde la respuesta inicial es el shell vacío de React/Vue, todo el contenido se hidrata client-side desde una API que no aparece en el HTML inicial. Endpoints triviales `/api/puestos`, `/data.json`, etc. devuelven el mismo shell (10402B). **Reconfirmado bloqueador en 2026-05-24:** intento de inspección con WebFetch falla con "Claude Code is unable to fetch from convocatorias.rtve.es" — no es accesible para análisis remoto, y sin abrir DevTools en navegador real no podemos rastrear la primera request XHR/fetch.~~ ✅ **Resuelto (2026-06-26, `v0.5.2`).** El error era mirar la página equivocada: `convocatorias.rtve.es` (portal de inscripción) sí es una SPA inescrutable, pero la página **corporativa** `https://www.rtve.es/corporacion/ofertas-empleo/` es **HTML server-rendered** (alcanzable con `requests`) y lista cada convocatoria (`article.cell > div.mod.notic_mod`: pretitle + maintitle + entradilla + enlace a bases PDF). El disparador fue un empleo perdido: la **Convocatoria 1/2026 Banco de Datos de Enfermería de Empresa** (bolsa de contratos temporales — ese tipo de proceso NO va al BOE, por eso la cobertura indirecta no la pillaba). Nueva fuente `vigia/sources/rtve.py` (extra_source de enfermería); emite por FAST_KEYWORDS y el extractor filtra (`enfermería de empresa` ya era STRONG). 5 tests en `test_rtve.py`.
...
- ~~Parser dedicado del portal de RTVE~~ ✅ Hecho (2026-06-26, `v0.5.2`): `vigia/sources/rtve.py` parsea la página corporativa server-rendered (la SPA `convocatorias.rtve.es` era la página equivocada). Ver nota de RTVE arriba.
...
- ~~**Replicar fix de timeouts en `ciemat.py`**~~ ✅ Resuelto (2026-05-24). `DETAIL_FETCH_TIMEOUT` subido de 20→30s en `vigia/sources/ciemat.py` siguiendo el mismo patrón que `c4d082a` aplicó en `comunidad_madrid`. Cierra los 3 Read timeouts del dry-run [run 25387900882](https://github.com/tragabytes/vigia-enfermeria/actions/runs/25387900882) (`detalle 2381`, `2380`, `2292`).
- ~~**`recalcular_fechas_universidades_madrid` análogo al de Comunidad de Madrid**~~ ✅ Ya estaba en el PR [#1](https://github.com/tragabytes/vigia-enfermeria/pull/1) del propio ultra-review (commit `686d6c2`). Implementado en [`maintenance.py:91`](vigia/maintenance.py:91) con 5 tests en `test_maintenance.py`, cableado en `main.py:149`. La nota suelta de esta sección quedó desactualizada respecto al propio ultra-review.
