...
function renderIntel() {
  ...
  // 4c. Calendar heatmap - 365 days ending today
  const end = new Date(DATA.meta.last_run_at.slice(0,10) + 'T00:00:00Z');
  const days = 371; // pad to start on Monday
  const counts = new Map();
  DATA.items.forEach(it => {
    const k = it.first_seen_at.slice(0,10);
    counts.set(k, (counts.get(k)||0) + 1);
  });
  // simulate plausible activity (seed with deterministic-ish noise around real items)
  const seedRand = mulberry32(1247);
  let heatHTML = '';
  const monthsRow = [];
  let lastMonth = -1;
  for (let i = days - 1; i >= 0; i--) {
    ...
    const realN = counts.get(k) || 0;
    let synth = 0;
    if (realN === 0) {
      const r = seedRand();
      if (r > 0.78) synth = 1;
      else if (r > 0.94) synth = 2;
    }
    const n = realN + synth;
    let l = 0;
    if (n >= 4) l = 4;
    ...
    heatHTML += `<div class="heat-cell" data-l="${l}" title="${k} · ${n}"></div>`;
    if (d.getUTCDate() === 1) {
      monthsRow.push({col: Math.floor((days-1-i)/7), name: d.toLocaleString('en', {month: 'short', timeZone: 'UTC'}).toUpperCase()});
    }
  ...
  for (let i = 29; i >= 0; i--) {
    ...
    const realN = counts.get(k) || 0;
    const r = seedRand();
    sparkVals.push(realN + (r > 0.85 ? 1 : 0));
  }
  const w = 360, h = 36, pad = 2;
  const mx = Math.max(1, ...sparkVals);
  const stepX = (w - pad*2) / (sparkVals.length - 1);
  const pts = sparkVals.map((v,i) => {
    const x = pad + i * stepX;
    const y = h - pad - (v / mx) * (h - pad*2);
    return `${x.toFixed(1)},${y.toFixed(1)}`;
  }).join(' ');
  const sum30 = sparkVals.reduce((a,b)=>a+b,0);
  $('#sparkline').innerHTML = `
    <div class="spark-row">
      <svg class="spark" viewBox="0 0 ${w} ${h}" preserveAspectRatio="none">
        <polyline points="${pts}" fill="none" stroke="#39ff14" stroke-width="1.4" stroke-linejoin="round"/>
        <polyline points="0,${h} ${pts} ${w},${h}" fill="rgba(57,255,20,0.10)" stroke="none"/>
      </svg>
      <div class="spark-stat">
        <b>${sum30}</b> HITS · LAST 30D
      </div>
    </div>
  `;
}
