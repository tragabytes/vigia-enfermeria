...
>
> **Fix — `vigia-core@v0.4.4` (2026-06-13):** el backfill de v0.4.3 destapó una regresión — al re-enriquecer, BOE dio timeout desde el runner (throttling por ráfaga), 24 items se re-enriquecieron sin cuerpo y `update_enrichment` **machacó** deadlines buenos (cobertura 16/55→12/55, daño práctico nulo: todos cerrados). Arreglo en `enrich_pending`: si un item ya tiene enriquecimiento estructurado (`enriched_version >= 2`) y no se recarga su body, **se preserva** (no se re-enriquece degradado) + cortesía entre descargas. Latente (protege futuros backfills). PR: [vigia-core#8](https://github.com/tragabytes/vigia-core/pull/8).
